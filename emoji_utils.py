"""
==================================================
🌌 Ryhavean Userbot — Premium Emoji Utility (Telethon)
==================================================
...
# ============================================================
# Köməkçi funksiyalar / Helper API
# ============================================================
def get_emoji_id(emoji: str):
    """Verilmiş emoji üçün premium document id qaytarır (yoxdursa None)."""
    return PREMIUM_EMOJI_MAP.get(emoji)


def emoji(char: str, fallback: str | None = None) -> str:
    """HTML custom-emoji teqi qaytarır: <emoji id="...">🔥</emoji>"""
    doc_id = PREMIUM_EMOJI_MAP.get(char)
    visible = fallback if fallback is not None else char
    if not doc_id:
        return visible
    return f'<emoji id="{doc_id}">{visible}</emoji>'


def random_song_emoji() -> tuple:
    """(emoji_char, document_id) — .song üçün təsadüfi premium emoji."""
    idx = random.randrange(len(SONG_EMOJI_CHAR))
    return SONG_EMOJI_CHAR[idx], SONG_PREMIUM_EMOJI_IDS[idx]


def premium_text(text: str):
    """(mətn, entities) cütü — hazır göndərmək üçün."""
    return apply_premium_emojis(text, None, 'html')


def add_emoji(char: str, document_id: int) -> None:
    """Yeni premium emoji ID əlavə et (runtime)."""
    PREMIUM_EMOJI_MAP[str(char)] = int(document_id)


def load_extra_ids(mapping: dict) -> int:
    """data/custom_emoji_ids.json kimi xarici mənbədən ID-ləri yüklə."""
    added = 0
    for key, value in (mapping or {}).items():
        try:
            if isinstance(value, (list, tuple)) and value:
                value = value[0]
            if key not in PREMIUM_EMOJI_MAP:
                PREMIUM_EMOJI_MAP[str(key)] = int(value)
                added += 1
        except Exception:
            continue
    return added
