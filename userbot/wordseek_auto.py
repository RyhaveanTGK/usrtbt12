...
async def wait_for_bot_response(client, chat_id: int, timeout: int = 10):
    """
    ...
    try:
        message = await client.listen.Message(
            chat=chat_id, func=_is_game_bot_response, timeout=timeout,
        )
        if message:
...
async def play_game_loop(client, chat_id: int):
    """
...
# Qeyd: eyni qrupda yalnız ilk uyğun handler işləyir, ona görə bu
...
@on_event(events.NewMessage(incoming=True, func=lambda e: e.is_group))
async def auto_play_handler(event):
    client = event.client
    message = event
    ...
    is_private_bot = message.chat.type == 'private' and (message.chat.username or '').lower() in ['wordseekbot', 'crocodilegameenn_bot']
    
    # 3. Any valid length guess word in a group chat where a game was recently started
    is_game_active_in_group = False
    if not is_private_bot and is_valid_len_word and message.chat.type in ('group', 'supergroup'):
        try:
...
@ryhavean_cmd('gameinfo')
async def show_game_info(event):
    client = event.client
    message = event
...
@ryhavean_cmd('wordseek')
async def wordseek_info(event):
    client = event.client
    message = event
