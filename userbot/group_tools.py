...
@ryhavean_cmd("power", group_only=True)
@retry()
    ...
    if len(command_parts) >= 2:
        ...
        try:
            await client.promote_chat_member(
                chat_id,
                user_id, privileges=ChatAdminRights(
                **permissions,)
            )
...
@ryhavean_cmd("inv", group_only=True)
@retry()
async def inv(event):
    client = event.client
    message = event
...
@ryhavean_cmd("admins", group_only=True)
@retry()
async def adminlist(event):
    ...
    async for a in client.get_chat_members(chat, filter=ChatMembersFilter.ADMINISTRATORS):
        try:
...
@ryhavean_cmd("leave", group_only=True)
@retry()
async def leave_command(event):
    ...
    try:
        ...
        await client.send_message(client.uid, styled_error(f"Failed to leave `{chat_id}`: {e}"))
