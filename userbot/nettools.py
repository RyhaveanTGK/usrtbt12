...
@ryhavean_cmd("pingurl", sudo=True)
async def http_ping(event):
    client = event.client
    message = event
...
@ryhavean_cmd("tcp", sudo=True)
async def tcp_test(event):
    client = event.client
    message = event
...
# Async speedtest
@ryhavean_cmd("speed", sudo=True)
async def async_speedtest(event):
    client = event.client
    message = event
...
# Calculator command
@ryhavean_cmd(["calc", "calculate"], sudo=True)
