...
async def _safe_edit(message: Message, text: str):
    """Edit a message, tolerating MessageNotModifiedError and similar edit errors."""
    try:
...
@ryhavean_cmd(["askclear", "askreset"])
@retry()
async def ask_clear_handler(event):
    client = event.client
    message = event
...
@ryhavean_cmd("askmodel")
@retry()
async def ask_model_handler(event):
    client = event.client
    message = event
