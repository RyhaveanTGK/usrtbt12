...
def _no_privileges():
    ...
    `ChatAdminRights()` defaults `can_manage_chat` to True, so a demote built from
    ...
    return ChatAdminRights(**{attr: False for attr in sorted(attrs)})
...
async def _moderate(client, message, budget, tool_input):
    ...
    if getattr(client, "me", None) is not None and user.id == client.uid:
...
async def _message_action(client, message, budget, tool_input):
    ...
    own_message = (
        ...
        and replied.from_user.id == client.uid
    )
...
def _coerce_enum(enum_name, value):
    """Mətn dəyərini uyğun enum üzvünə çevirir."""
...
def _coerce(param, value, message, annotations=None):
    ...
    parameter is annotated as a datetime or an enum. Anything else passes
