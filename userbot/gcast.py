...
async def gcast_handler(event):
    ...
    try:
        ...
        except (ChatForwardsRestricted, FileReferenceExpired, MessageIdInvalidError):
           if reply_msg.media:
        ...
        async for dialog in client.get_dialogs():
          if flag in ["-all", "-pvt", "-grp"]:
            ...
            should_send = (
                flag == "-all"
                or (flag == "-pvt" and dialog.chat.type == 'private')
                or (flag == "-grp" and dialog.chat.type in ['group', 'supergroup'])
            )
            if not should_send:
                continue
            try:
