...
def is_music_on():
    def decorator(func):
        ...
        async def wrapper(client, message):
            try:
                user_data = user_sessions.find_one({"user_id": client.uid})
                ...
                logger.error(f"[MUSIC] is_music_on decorator error for user {client.uid}: {e}")
                return
        return wrapper
    return decorator

async def is_active_chat(client, chat_id):
    if hasattr(client, 'me') and client.uid in active:
        return chat_id in active[client.uid]
    return False

async def add_active_chat(client, chat_id):
    if hasattr(client, 'me'):
        if client.uid not in active:
            active[client.uid] = []
        if chat_id not in active[client.uid]:
            active[client.uid].append(chat_id)
...
async def download_media_with_progress(client, msg, media_msg, type_of):
    ...
    session_name = f'user_{client.uid}'
    ...
    try:
        ...
        logger.error(f"[MUSIC] Download error for user {client.uid}: {e}")
        return None
    ...
    try:
            ...
            try:
              if random.choices([True, False], weights=[1, 20])[0]:
                ...
                logger.debug(f"[MUSIC] Progress update error for user {client.uid}: {e}")

    except Exception as e:
        logger.error(f"[MUSIC] Progress bar error for user {client.uid}: {e}")
...
forceplay = False):
    ...
    if forceplay:
        song_queue = queues.get(f"dic_{client.uid}")
...
async def dend(client, update, channel_id=None):
    song_queue_key = f"dic_{client.uid}"
    song_queue = queues.get(song_queue_key)
    if song_queue is None:
        logger.info(f"Song queue not found for user: {client.uid}")
        call_py = songs_client.get(client.uid)
        if call_py:
    ...
    call_py = songs_client.get(client.uid)
    if not call_py:
        logger.info(f"Call client not found for user: {client.uid}")
        return
...
@ryhavean_cmd(["play", "vplay", "playforce", "vplayforce"])
@is_music_on()
async def play_handler_func(event):
    client = event.client
    message = event
    ...
    song_queue = queues.get(f"dic_{client.uid}")
    if not song_queue:
        queues[f"dic_{client.uid}"] = {}
        song_queue = queues[f"dic_{client.uid}"]
    ...
    if message.reply_to_message and message.reply_to_message.media:
        ...
        logger.info(f"[MUSIC] YouTube search result for user {client.uid}: {title}")
        if not youtube_link:
            try:
                await massage.edit(
...
@ryhavean_cmd("end")
@is_music_on()
async def end_handler_func(event):
    client = event.client
    message = event
    # Message deletion removed - keeping original message
    try:
...
@ryhavean_cmd("skip")
@is_music_on()
async def skip_handler_func(event):
    client = event.client
    message = event
    # Message deletion removed - keeping original message
    try:
...
@ryhavean_cmd("pause")
@is_music_on()
async def pause_handler_func(event):
    client = event.client
    message = event
    try:
        call_py = songs_client.get(client.uid)
...
@ryhavean_cmd("resume")
@is_music_on()
async def resume_handler_func(event):
    client = event.client
    message = event
    try:
        call_py = songs_client.get(client.uid)
...
@ryhavean_cmd("loop")
@is_music_on()
async def loop_handler_func(event):
    client = event.client
    message = event
    # Message deletion removed - keeping original message
