...
def create_custom_filter(event):
    try:
        if not event.sender_id:
            return False
        pattern = get_user_data(event.sender_id).get("save_com", "^(Wow|wow)$")
        return bool(re.match(pattern, event.raw_text or ""))
    except Exception:
        return False
...
@on_event(events.NewMessage(outgoing=True, func=lambda e: e.is_private))
async def auto_download_media(event):
    client = event.client
    message = event
    """
    ...
    try:
        ...
        sender_id = sender.id
        is_self_message = str(sender_id) == str(client.uid)
        ...
        try:
            ...
            if app_me_id and app_me_id != client.uid:
                kwargs = {
                    'chat_id': app_me_id,
                    media_type: file_path
                }
