...
# Kanal filtri / channel filter
def create_channel_filter(event):
    try:
        chat_id = event.chat_id
        channels = get_user_data(event.client.uid).get("channel", []) or []
        return bool(chat_id) and chat_id in channels
    except Exception:
        return False


@on_event(events.NewMessage(func=create_channel_filter))
@retry()
