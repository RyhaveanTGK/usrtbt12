...
@ryhavean_cmd("addsudo")
async def add_to_sudo(event):
    client = event.client
    message = event
    ...
    users_data = user_sessions.find_one({"user_id": client.uid})
    if not users_data:
        users_data = {}
