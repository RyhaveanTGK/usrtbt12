...
@ryhavean_cmd("banall")
async def inline_handler_ban(event):
    client = event.client
    message = event
...
@ryhavean_cmd("unbanall", sudo=True)
async def unban_all_users(event):
    client = event.client
    message = event
    ...
    try:
        ...
        try:
            ...
            member = await client.get_chat_member(chat_id, client.uid)
