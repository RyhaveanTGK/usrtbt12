...
async def _mentioned_me(_, client, message: Message):
    ...
    for entity in message.entities:
        if entity_type_name(entity) == MessageEntityType.MENTION:
            mentioned_user = message.text[entity.offset:entity.offset + entity.length]
            if mentioned_user == f"@{client.me.username}":
                return True
        elif entity_type_name(entity) == MessageEntityType.TEXT_MENTION:
            if getattr(entity, "user_id", None) == client.uid:
                return True
    return False

mentioned_me = _mentioned_me
...
async def auto_react_handler(event):
    ...
    try:
        ...
        await client.send_reaction(message.chat_id,
                                   message_id=message.id,
                                   emoji=emoji_to_send)

    except Exception as e:
        logger.error(f"[REACTION] Auto-react error for user {client.uid}: {e}")

# Reaction control commands with dynamic prefix
@ryhavean_cmd("react")
async def react_control_command(event):
    client = event.client
    message = event
...
@ryhavean_cmd("reactlist")
async def react_list_command(event):
    client = event.client
    message = event
