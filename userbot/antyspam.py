...
async def handle_user(event):
    ...
    # Render the settings menu with emojis
        ...
        if user_count == 1:
            ...
            os.makedirs(user_dir, exist_ok=True)
            photu = None
            async for photo in client.get_chat_photos(client.uid):
                photu = photo.file_id
            logo = gvarstatus(client.uid, "ALIVE_LOGO") or (await client.download_media(client.me.photo.big_file_id, f"{user_dir}/{'logo.mp4' if client.me.photo.has_animation else 'logo.jpg'}") if client.me.photo else "userbot.jpg")
            alive_logo = logo
            ...
            greet_message = gvarstatus(client.uid, "WELCOME") or f"""<blockquote>{bold_cool(f"👋 Warm greetings, {full_name}! Welcome to my private message.")}</blockquote>
...
@ryhavean_cmd("approve", private_only=True)
@retry()
...
@ryhavean_cmd("disapprove", private_only=True)
@retry()
async def disapprove_user(event):
    ...
    if user_data:
        ...
        logger.warning(f"No data found for user_id {client.uid}.")
        await message.edit_text("No data found for the bot user.")

@ryhavean_cmd("rmall", private_only=True)
@retry()
async def remove_all_whitelisted_users(event):
    client = event.client
    message = event
...
@ryhavean_cmd("rstall", private_only=True)
@retry()
async def reset_all_users_count(event):
    client = event.client
    message = event
...
@ryhavean_cmd("rst", private_only=True)
@retry()
async def reset_user_count(event):
    client = event.client
    message = event
