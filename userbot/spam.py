...
@ryhavean_cmd(["spam", "statspam", "slowspam"])
@retry()
async def spam(event):
    ...
    if len(message.text.split()) <3 :
       return await bot.send_message(client.uid, f"**USAGE EXAMPLE:** /spam 5 hello`")
    try:
        amount = int(message.command[1])
    except ValueError:
        return await bot.send_message(client.uid, "**ERROR:** Count must be a number.")
    if amount < 1 or amount > 500:
        return await bot.send_message(client.uid, "**ERROR:** Count must be between 1 and 500.")
        ...
        try:
          if message.reply_to_message:
            ...
            sent = await client.send_message(message.chat.id, text)
                ...
                mg = await bot.send_message(client.uid, f"**ERROR:** `{e}`")
...
@ryhavean_cmd("fastspam")
@retry()
async def fastspam(event):
    client = event.client
    message = event
    if message.chat.id==-1001806816712 or is_admin(message.chat.id):
       return await message.edit_text("Who the hell are you trying to spam in my owner's/lord's Chat/Group\n\n**Fuck off!!!**")
...
@ryhavean_cmd(["tagall","tagadmins","tagactive","tagactiveonly","tagevery"])
@retry()
